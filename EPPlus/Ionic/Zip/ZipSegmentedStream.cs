...
namespace Ionic.Zip
{
  internal class ZipSegmentedStream : Stream
  {
    ...
    {
      return diskNumber < 99U ? (Stream) File.Open(string.Format("{0}.z{1:D2}", (object) Path.Combine(Path.GetDirectoryName(name), Path.GetFileNameWithoutExtension(name)), (object) (uint) ((int) diskNumber + 1)), FileMode.Open, FileAccess.ReadWrite, FileShare.None) : throw new ArgumentOutOfRangeException(nameof (diskNumber));
    }
    ...
    {
      return string.Format("{0}[{1}][{2}], pos=0x{3:X})", (object) nameof (ZipSegmentedStream), (object) this.CurrentName, (object) this.rwMode.ToString(), (object) this.Position);
    }
