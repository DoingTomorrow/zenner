...
// Type: OfficeOpenXml.FormulaParsing.Excel.Functions.Math.Average
  ...
  public class Average : HiddenValuesHandlingFunction
  {
      ...
      bool isInArray = false)
    {
      ...
      else
      {
        ...
        else if (arg.Value is string && !ConvertUtil.IsNumericString(arg.Value) && !isInArray)
    ...
    private double? GetNumericValue(object obj, bool isInArray)
    {
      ...
      if (obj is bool && !isInArray)
        return new double?(ConvertUtil.GetValueDouble(obj));
      return ConvertUtil.IsNumericString(obj) ? new double?(double.Parse(obj.ToString(), (IFormatProvider) CultureInfo.InvariantCulture)) : new double?();
    }
  }
}
