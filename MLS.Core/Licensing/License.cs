// Decompiled with JetBrains decompiler
// Type: MLS.Core.Model.Licensing.License
// Assembly: MLS.Core, Version=1.0.0.0, Culture=neutral, PublicKeyToken=null
// MVID: 7FC8C31B-A3E8-40E1-84D2-E43683A764BC
// Assembly location: F:\tekst\DoingTomorrow\Zenner_Software\program_filer\MLS.Core.dll

using System;

#nullable disable
namespace MLS.Core.Model.Licensing
{
  public class License
  {
    public virtual int Id { get; set; }

    public virtual LicenseBulk LicenseBulk { get; set; }

    public virtual DateTime StartDate { get; set; }

    public virtual DateTime? EndDate { get; set; }

    public virtual string HardwareKey { get; set; }

    public virtual string HardwareName { get; set; }

    public virtual string ApplicationVersion { get; set; }

    public virtual string ApplicationVersionManual { get; set; }

    public virtual byte[] LicenseFile { get; set; }

    public virtual bool Active { get; set; }
  }
}
