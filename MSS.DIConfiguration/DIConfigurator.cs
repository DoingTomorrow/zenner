...
// Type: MSS.DIConfiguration.DIConfigurator
...
namespace MSS.DIConfiguration
{
  public class DIConfigurator
  {
    ...
    {
      if (DIConfigurator.kernel != null)
        return DIConfigurator.kernel;
      DIConfigurator.kernel = (IKernel) new StandardKernel(Array.Empty<INinjectModule>());
      List<INinjectModule> m = new List<INinjectModule>()
      {
        (INinjectModule) new RepositoryModule(),
        (INinjectModule) new NHibernateModule()
      };
      DIConfigurator.kernel.Load((IEnumerable<INinjectModule>) m);
      return DIConfigurator.kernel;
    }
  }
}
