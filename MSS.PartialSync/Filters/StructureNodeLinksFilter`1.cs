...
// Type: MSS.PartialSync.Filters.StructureNodeLinksFilter`1
...
{
  public class StructureNodeLinksFilter<T> : GenericEntityFilter<T> where T : StructureNodeLinks
  {
    private Expression DefineFilters(Guid id)
    {
      Guid guid = id;
      Expression<Func<T, bool>> expression = (Expression<Func<T, bool>>) (c => c.RootNode.Id == id);
      (Expression<Func<T, bool>>) (c => c.EndDate == new DateTime?());
