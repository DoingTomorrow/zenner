...
// Type: MSS.PartialSyncData.Repository.StructuresNodeRepository
...
namespace MSS.PartialSyncData.Repository
{
  public class StructuresNodeRepository(ISession session) : 
    MSS.PartialSyncData.Repository.Repository<StructureNode>(session),
    IStructureNodeRepository
  {
      ...
      out List<string> duplicateMeterSerialNumbers)
    {
      ...
      List<StructureNodeLinks> structureLinksWithNodes = list;
      ...
      if ((nullable.GetValueOrDefault() == structureTypeEnum ? (nullable.HasValue ? 1 : 0) : 0) != 0 && meterList.Count > 0)
      {
        ...
        string queryString2 = "Select SNGroups.[SerialNumber] from (Select m.[SerialNumber], COUNT(m.[SerialNumber]) as cnt   From [t_Meter] as m   where m.[SerialNumber] In (" + meterSerialNumbers.ToString().TrimEnd(',') + ")  and m.[IsDeactivated] = 0   group by m.[SerialNumber]) as SNGroups   Where cnt > 1";
        ISQLQuery sqlQuery2 = session.CreateSQLQuery(queryString2);
        duplicateMeterSerialNumbers = sqlQuery2.List<string>().ToList<string>();
      }
      string queryString3 = "Select distinct {l.*}  from [t_Location] as l \r\n                    inner join [t_StructureNode] on l.[Id] = [t_StructureNode].[EntityId]  \r\n                    inner join [t_StructureNodeLinks]  on [t_StructureNode] .Id = [t_StructureNodeLinks].NodeId  \r\n                    where [t_StructureNodeLinks].Id In (" + linkIds.ToString().TrimEnd(',') + ") and [t_StructureNode].[EndDate] is null and   [t_StructureNodeLinks].[StructureType] = '" + (object) structureType + "'";
      ISQLQuery sqlQuery3 = session.CreateSQLQuery(queryString3);
      sqlQuery3.AddEntity("l", typeof (Location));
      IList<Location> source1 = sqlQuery3.List<Location>();
      string queryString4 = rootNodeId != Guid.Empty ? string.Format("Select distinct t.* from [t_Tenant] as t \r\n                    inner join [t_StructureNode] on t.[Id] = [t_StructureNode].[EntityId]  \r\n                    inner join [t_StructureNodeLinks]  on [t_StructureNode] .Id = [t_StructureNodeLinks].NodeId  \r\n                    where [t_StructureNodeLinks].Id In (\r\n                            select snl.Id from t_StructureNodeLinks as snl\r\n                            where snl.StructureType = '{0}' and snl.RootNodeId = '{1}' AND snl.EndDate IS NULL)", (object) structureType, (object) rootNodeId) : string.Format("Select distinct t.* from [t_Tenant] as t \r\n                    inner join [t_StructureNode] on t.[Id] = [t_StructureNode].[EntityId]  \r\n                    inner join [t_StructureNodeLinks]  on [t_StructureNode] .Id = [t_StructureNodeLinks].NodeId  \r\n                    where [t_StructureNodeLinks].Id In (\r\n                            select snl.Id from t_StructureNodeLinks as snl\r\n                            where snl.StructureType = '{0}' AND snl.EndDate IS NULL)", (object) structureType);
      ISQLQuery sqlQuery4 = session.CreateSQLQuery(queryString4);
      sqlQuery4.AddEntity("t", typeof (Tenant));
      IList<Tenant> source2 = sqlQuery4.List<Tenant>();
      string queryString5 = "Select distinct {m.*}  from [t_Minomat] as m \r\n                    inner join [t_StructureNode] on m.[Id] = [t_StructureNode].[EntityId]  \r\n                    inner join [t_StructureNodeLinks]  on [t_StructureNode] .Id = [t_StructureNodeLinks].NodeId  \r\n                    where [t_StructureNodeLinks].Id In (" + linkIds.ToString().TrimEnd(',') + ") and [t_StructureNode].[EndDate] is null and [t_StructureNodeLinks].[StructureType] = '" + (object) structureType + "'";
      ISQLQuery sqlQuery5 = session.CreateSQLQuery(queryString5);
      sqlQuery5.AddEntity("m", typeof (Minomat));
      IList<Minomat> source3 = sqlQuery5.List<Minomat>();
      entitiesDictionary.AddRange<Guid, object>(meterList.ToDictionary<Meter, Guid, object>((Func<Meter, Guid>) (x => x.Id), (Func<Meter, object>) (x => (object) x)));
