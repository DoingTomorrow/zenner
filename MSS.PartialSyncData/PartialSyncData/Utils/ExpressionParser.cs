...
// Type: MSS.PartialSyncData.Utils.ExpressionParser
...
namespace MSS.PartialSyncData.Utils
{
  internal class ExpressionParser
  {
    ...
    private Expression ParseUnary()
    {
      ...
      Expression unary1 = this.ParseUnary();
      Expression unary2;
      if (token.id == ExpressionParser.TokenId.Minus)
      {
        this.CheckAndPromoteOperand(typeof (ExpressionParser.INegationSignatures), token.text, ref unary1, token.pos);
        unary2 = (Expression) Expression.Negate(unary1);
      }
      else
      {
        this.CheckAndPromoteOperand(typeof (ExpressionParser.INotSignatures), token.text, ref unary1, token.pos);
        unary2 = (Expression) Expression.Not(unary1);
      }
      return unary2;
    }
    ...
    {
      Expression primary = this.ParsePrimaryStart();
      while (true)
      {
        ...
        else if (this.token.id == ExpressionParser.TokenId.OpenBracket)
    ...
    private Expression ParseRealLiteral()
    {
      ...
      {
        float result;
        if (float.TryParse(text.Substring(0, text.Length - 1), out result))
          obj = (object) result;
      }
      else
      {
    ...
    private Expression ParseIdentifier()
    {
      ...
      object identifier1;
      if (ExpressionParser.keywords.TryGetValue(this.token.text, out identifier1))
      {
        if (identifier1 is Type)
          return this.ParseTypeAccess((Type) identifier1);
        if (identifier1 == (object) ExpressionParser.keywordIt)
          return this.ParseIt();
        if (identifier1 == (object) ExpressionParser.keywordIif)
          return this.ParseIif();
        if (identifier1 == (object) ExpressionParser.keywordNew)
          return this.ParseNew();
        this.NextToken();
        return (Expression) identifier1;
      }
      if (this.symbols.TryGetValue(this.token.text, out identifier1) || this.externals != null && this.externals.TryGetValue(this.token.text, out identifier1))
      {
        if (!(identifier1 is Expression identifier2))
          identifier2 = (Expression) Expression.Constant(identifier1);
        else if (identifier2 is LambdaExpression lambda)
...
label_8:
      ...
      this.NextToken();
      Type type = DynamicExpression.CreateClass((IEnumerable<DynamicProperty>) properties);
      MemberBinding[] memberBindingArray = new MemberBinding[properties.Count];
      for (int index = 0; index < memberBindingArray.Length; ++index)
        memberBindingArray[index] = (MemberBinding) Expression.Bind((MemberInfo) type.GetProperty(properties[index].Name), expressionList[index]);
    ...
    private Expression ParseMemberAccess(Type type, Expression instance)
    {
      ...
      else
      {
        ...
        return (object) (propertyOrField as PropertyInfo) != null ? (Expression) Expression.Property(instance, (PropertyInfo) propertyOrField) : (Expression) Expression.Field(instance, (FieldInfo) propertyOrField);
      }
    }
    ...
    private Expression[] ParseArgumentList()
    {
      this.ValidateToken(ExpressionParser.TokenId.OpenParen, "'(' expected");
      this.NextToken();
      Expression[] argumentList = this.token.id != ExpressionParser.TokenId.CloseParen ? this.ParseArguments() : new Expression[0];
      this.ValidateToken(ExpressionParser.TokenId.CloseParen, "')' or ',' expected");
      this.NextToken();
      return argumentList;
    }
      ...
      out MethodBase method)
    {
      ...
      foreach (Type selfAndBaseType in ExpressionParser.SelfAndBaseTypes(type))
      {
        int bestMethod = this.FindBestMethod(Enumerable.Cast<MethodBase>(selfAndBaseType.FindMembers(MemberTypes.Method, bindingAttr, Type.FilterNameIgnoreCase, (object) methodName)), args, out method);
        if (bestMethod != 0)
          return bestMethod;
      }
    ...
    private int FindIndexer(Type type, Expression[] args, out MethodBase method)
    {
      foreach (Type selfAndBaseType in ExpressionParser.SelfAndBaseTypes(type))
      {
        ...
        {
          int bestMethod = this.FindBestMethod(Enumerable.OfType<PropertyInfo>(defaultMembers).Select<PropertyInfo, MethodBase>((Func<PropertyInfo, MethodBase>) (p => (MethodBase) p.GetGetMethod())).Where<MethodBase>((Func<MethodBase, bool>) (m => m != (MethodBase) null)), args, out method);
          if (bestMethod != 0)
            return bestMethod;
        }
      }
    ...
    {
      return left.Type == typeof (string) ? (Expression) Expression.GreaterThan(this.GenerateStaticMethodCall("Compare", left, right), (Expression) Expression.Constant((object) 0)) : (Expression) Expression.GreaterThan(left, right);
    }

    private Expression GenerateGreaterThanEqual(Expression left, Expression right)
    {
      return left.Type == typeof (string) ? (Expression) Expression.GreaterThanOrEqual(this.GenerateStaticMethodCall("Compare", left, right), (Expression) Expression.Constant((object) 0)) : (Expression) Expression.GreaterThanOrEqual(left, right);
    }

    private Expression GenerateLessThan(Expression left, Expression right)
    {
      return left.Type == typeof (string) ? (Expression) Expression.LessThan(this.GenerateStaticMethodCall("Compare", left, right), (Expression) Expression.Constant((object) 0)) : (Expression) Expression.LessThan(left, right);
    }

    private Expression GenerateLessThanEqual(Expression left, Expression right)
    {
      return left.Type == typeof (string) ? (Expression) Expression.LessThanOrEqual(this.GenerateStaticMethodCall("Compare", left, right), (Expression) Expression.Constant((object) 0)) : (Expression) Expression.LessThanOrEqual(left, right);
    }

    private Expression GenerateAdd(Expression left, Expression right)
    {
      return left.Type == typeof (string) && right.Type == typeof (string) ? this.GenerateStaticMethodCall("Concat", left, right) : (Expression) Expression.Add(left, right);
    }
    ...
    private string GetIdentifier()
    {
      ...
      string identifier = this.token.text;
    ...
    private static Dictionary<string, object> CreateKeywords()
    {
      ...
      keywords.Add("true", (object) ExpressionParser.trueLiteral);
      keywords.Add("false", (object) ExpressionParser.falseLiteral);
      keywords.Add("null", (object) ExpressionParser.nullLiteral);
