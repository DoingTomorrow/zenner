...
namespace Standard
{
  internal static class NativeMethods
  {
    ...
    public static void AllowSetForegroundWindow() => NativeMethods.AllowSetForegroundWindow(-1);
    ...
    {
      POINT lpPoint;
      if (!NativeMethods._GetCursorPos(out lpPoint))
        HRESULT.ThrowLastError();
      return lpPoint;
    }
    ...
    {
      MONITORINFO lpmi = new MONITORINFO();
      if (!NativeMethods._GetMonitorInfoW(hMonitor, lpmi))
        throw new Win32Exception();
      return lpmi;
    }
    ...
    public static WINDOWINFO GetWindowInfo(IntPtr hWnd)
    {
      ...
      if (!NativeMethods._GetWindowInfo(hWnd, ref pwi))
        HRESULT.ThrowLastError();
      return pwi;
    }
    ...
    {
      NativeMethods._SHAddToRecentDocsObj(SHARD.APPIDINFO, (object) info);
    }

    public static void SHAddToRecentDocs(SHARDAPPIDINFOIDLIST infodIdList)
    {
      NativeMethods._SHAddToRecentDocsObj(SHARD.APPIDINFOIDLIST, (object) infodIdList);
    }
