// Decompiled with JetBrains decompiler
// Type: Antlr.Runtime.Tree.RewriteRuleSubtreeStream
// Assembly: NHibernate, Version=3.3.1.4000, Culture=neutral, PublicKeyToken=aa95f207798dfdb4
// MVID: F2FE07FE-F4FA-4811-8A3A-0A4855BEE49E
// Assembly location: F:\tekst\DoingTomorrow\Zenner_Software\program_filer\NHibernate.dll

using System;
using System.Collections;
using System.Collections.Generic;

#nullable disable
namespace Antlr.Runtime.Tree
{
  internal class RewriteRuleSubtreeStream : RewriteRuleElementStream<object>
  {
    public RewriteRuleSubtreeStream(ITreeAdaptor adaptor, string elementDescription)
      : base(adaptor, elementDescription)
    {
    }

    public RewriteRuleSubtreeStream(
      ITreeAdaptor adaptor,
      string elementDescription,
      object oneElement)
      : base(adaptor, elementDescription, oneElement)
    {
    }

    public RewriteRuleSubtreeStream(
      ITreeAdaptor adaptor,
      string elementDescription,
      IList<object> elements)
      : base(adaptor, elementDescription, elements)
    {
    }

    [Obsolete("This constructor is for internal use only and might be phased out soon. Use instead the one with IList<T>.")]
    public RewriteRuleSubtreeStream(
      ITreeAdaptor adaptor,
      string elementDescription,
      IList elements)
      : base(adaptor, elementDescription, elements)
    {
    }

    public object NextNode()
    {
      return this.FetchObject((RewriteRuleSubtreeStream.ProcessHandler) (o => this.adaptor.DupNode(o)));
    }

    private object FetchObject(RewriteRuleSubtreeStream.ProcessHandler ph)
    {
      return this.RequiresDuplication() ? ph(this._Next()) : this._Next();
    }

    private bool RequiresDuplication()
    {
      int count = this.Count;
      if (this.dirty)
        return true;
      return this.cursor >= count && count == 1;
    }

    public override object NextTree()
    {
      return this.FetchObject((RewriteRuleSubtreeStream.ProcessHandler) (o => this.Dup(o)));
    }

    private object Dup(object el) => this.adaptor.DupTree(el);

    private delegate object ProcessHandler(object o);
  }
}
