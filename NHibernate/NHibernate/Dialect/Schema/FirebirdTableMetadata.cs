...
// Type: NHibernate.Dialect.Schema.FirebirdTableMetadata
...
{
  public class FirebirdTableMetadata(DataRow rs, IDataBaseSchema meta, bool extras) : 
    AbstractTableMetadata(rs, meta, extras)
  {
    ...
    {
      return (IColumnMetadata) new FirebirdColumnMetadata(rs);
    }

    protected override IForeignKeyMetadata GetForeignKeyMetadata(DataRow rs)
    {
      return (IForeignKeyMetadata) new FirebirdForeignKeyMetadata(rs);
    }

    protected override IIndexMetadata GetIndexMetadata(DataRow rs)
    {
      return (IIndexMetadata) new FirebirdIndexMetadata(rs);
    }
  }
}
