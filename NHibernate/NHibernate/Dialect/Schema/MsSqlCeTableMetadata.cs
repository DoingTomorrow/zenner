...
// Type: NHibernate.Dialect.Schema.MsSqlCeTableMetadata
...
{
  public class MsSqlCeTableMetadata(DataRow rs, IDataBaseSchema meta, bool extras) : 
    AbstractTableMetadata(rs, meta, extras)
  {
    ...
    {
      return (IColumnMetadata) new MsSqlCeColumnMetadata(rs);
    }

    protected override IForeignKeyMetadata GetForeignKeyMetadata(DataRow rs)
    {
      return (IForeignKeyMetadata) new MsSqlCeForeignKeyMetadata(rs);
    }

    protected override IIndexMetadata GetIndexMetadata(DataRow rs)
    {
      return (IIndexMetadata) new MsSqlCeIndexMetadata(rs);
    }
  }
}
