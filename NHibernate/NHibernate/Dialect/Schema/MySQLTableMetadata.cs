...
// Type: NHibernate.Dialect.Schema.MySQLTableMetadata
...
{
  public class MySQLTableMetadata(DataRow rs, IDataBaseSchema meta, bool extras) : 
    AbstractTableMetadata(rs, meta, extras)
  {
    ...
    {
      return (IColumnMetadata) new MySQLColumnMetadata(rs);
    }

    protected override IForeignKeyMetadata GetForeignKeyMetadata(DataRow rs)
    {
      return (IForeignKeyMetadata) new MySQLForeignKeyMetadata(rs);
    }

    protected override IIndexMetadata GetIndexMetadata(DataRow rs)
    {
      return (IIndexMetadata) new MySQLIndexMetadata(rs);
    }
  }
}
