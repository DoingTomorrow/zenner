...
// Type: NHibernate.Dialect.Schema.PostgreSQLTableMetadata
...
{
  public class PostgreSQLTableMetadata(DataRow rs, IDataBaseSchema meta, bool extras) : 
    AbstractTableMetadata(rs, meta, extras)
  {
    protected override IColumnMetadata GetColumnMetadata(DataRow rs)
    {
      return (IColumnMetadata) new PostgreSQLColumnMetadata(rs);
    }
    ...
    {
      return (IForeignKeyMetadata) new PostgreSQLForeignKeyMetadata(rs);
    }

    protected override IIndexMetadata GetIndexMetadata(DataRow rs)
    {
      return (IIndexMetadata) new PostgreSQLIndexMetadata(rs);
    }
    ...
    {
      this.Catalog = Convert.ToString(rs["TABLE_CATALOG"]);
