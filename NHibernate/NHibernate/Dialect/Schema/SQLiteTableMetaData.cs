...
// Type: NHibernate.Dialect.Schema.SQLiteTableMetaData
...
{
  public class SQLiteTableMetaData(DataRow rs, IDataBaseSchema meta, bool extras) : 
    AbstractTableMetadata(rs, meta, extras)
  {
    protected override IColumnMetadata GetColumnMetadata(DataRow rs)
    {
      return (IColumnMetadata) new SQLiteColumnMetaData(rs);
    }
    ...
    {
      return (IForeignKeyMetadata) new SQLiteForeignKeyMetaData(rs);
    }

    protected override IIndexMetadata GetIndexMetadata(DataRow rs)
    {
      return (IIndexMetadata) new SQLiteIndexMetaData(rs);
    }
