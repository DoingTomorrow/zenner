...
// Type: NHibernate.Mapping.ByCode.Impl.SubclassMapper
...
namespace NHibernate.Mapping.ByCode.Impl
{
    ...
    IMinimalPlainPropertyContainerMapper
  {
    ...
    {
      HbmSubclass[] second = new HbmSubclass[1]
      {
        this.classMapping
      };
    ...
    {
      this.classMapping.discriminatorvalue = value != null ? value.ToString() : "null";
    }
    ...
    public void Join(string splitGroupId, Action<IJoinMapper> splitMapping)
    {
      ...
      if (!this.JoinMappers.TryGetValue(splitGroupId, out joinMapper))
      {
        ...
        this.classMapping.join = this.classMapping.join == null ? second : ((IEnumerable<HbmJoin>) this.classMapping.join).Concat<HbmJoin>((IEnumerable<HbmJoin>) second).ToArray<HbmJoin>();
      }
