...
// Type: NHibernate.Type.TimeSpanType
...
{
  [Serializable]
  public class TimeSpanType : PrimitiveType, IVersionType, IType, ICacheAssembler, ILiteralType
  {
    public TimeSpanType()
    ...
    public override object Get(IDataReader rs, int index)
    {
      try
      {
        return (object) new TimeSpan(Convert.ToInt64(rs[index]));
      }
      catch (Exception ex)
      {
    ...
    public override object Get(IDataReader rs, string name)
    {
      try
      {
        return (object) new TimeSpan(Convert.ToInt64(rs[name]));
      }
      catch (Exception ex)
      {
    ...
    {
      ((IDataParameter) st.Parameters[index]).Value = (object) ((TimeSpan) value).Ticks;
    }
