...
namespace Remotion.Linq.Clauses.ResultOperators
{
  public class CountResultOperator : ValueFromSequenceResultOperatorBase
  {
    ...
    {
      return new StreamedValue((object) input.GetTypedSequence<T>().Count<T>(), (StreamedValueInfo) this.GetOutputDataInfo((IStreamedDataInfo) input.DataInfo));
    }

    public override IStreamedDataInfo GetOutputDataInfo(IStreamedDataInfo inputInfo)
    {
      ArgumentUtility.CheckNotNullAndType<StreamedSequenceInfo>(nameof (inputInfo), (object) inputInfo);
      return (IStreamedDataInfo) new StreamedScalarValueInfo(typeof (int));
    }
