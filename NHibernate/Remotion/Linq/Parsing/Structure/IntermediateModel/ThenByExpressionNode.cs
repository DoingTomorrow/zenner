...
namespace Remotion.Linq.Parsing.Structure.IntermediateModel
{
  public class ThenByExpressionNode : MethodCallExpressionNodeBase
  {
    ...
    {
      MethodCallExpressionNodeBase.GetSupportedMethod<IOrderedQueryable<object>>((Expression<Func<IOrderedQueryable<object>>>) (() => default (IOrderedQueryable<object>).ThenBy<object, object>(default (Expression<Func<object, object>>)))),
      MethodCallExpressionNodeBase.GetSupportedMethod<IOrderedEnumerable<object>>((Expression<Func<IOrderedEnumerable<object>>>) (() => default (IOrderedEnumerable<object>).ThenBy<object, object>(default (Func<object, object>))))
    };
      ...
      ClauseGenerationContext clauseGenerationContext)
    {
      ...
      (this.GetOrderByClause(queryModel) ?? throw new ParserException("ThenByDescending expressions must follow OrderBy, OrderByDescending, ThenBy, or ThenByDescending expressions.")).Orderings.Add(new Ordering(this.GetResolvedKeySelector(clauseGenerationContext), OrderingDirection.Asc));
      return queryModel;
    }
