      ...
      BindingFlags bindingAttr)
    {
      while ((targetType = targetType.BaseType()) != (Type) null)
      {
        foreach (PropertyInfo property in targetType.GetProperties(bindingAttr))
        {
          ...
          else if (initialProperties.IndexOf<PropertyInfo>((Func<PropertyInfo, bool>) (p => p.Name == subTypeProperty.Name && p.IsVirtual() && p.GetBaseDefinition() != (MethodInfo) null && p.GetBaseDefinition().DeclaringType.IsAssignableFrom(subTypeProperty.GetBaseDefinition().DeclaringType))) == -1)
            initialProperties.Add(subTypeProperty);
        }
      }
    }
