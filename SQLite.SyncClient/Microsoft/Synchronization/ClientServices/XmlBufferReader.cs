...
namespace Microsoft.Synchronization.ClientServices
{
  public class XmlBufferReader
  {
    ...
    public static XmlBufferReader Empty => XmlBufferReader.EmptyBufferReader;

    public byte[] Buffer => this.buffer;

    public bool IsStreamed => this.stream != null;

    public bool EndOfFile => this.offset == this.offsetMax && !this.TryEnsureByte();
    ...
    public byte GetByte()
    {
      ...
      return offset < this.offsetMax ? this.buffer[offset] : this.GetByteHard();
    }

    public void SkipByte() => this.Advance(1);
    ...
    public byte[] GetBuffer(int count, out int outOffset)
    {
      ...
      return outOffset <= this.offsetMax - count ? this.buffer : this.GetBufferHard(count, out outOffset);
    }
    ...
    private bool TryEnsureBytes(int count)
    {
      ...
      {
        byte[] dst = new byte[Math.Max(val1, this.buffer.Length * 2)];
        System.Buffer.BlockCopy((Array) this.buffer, 0, (Array) dst, 0, this.offsetMax);
        this.buffer = dst;
    ...
    public void Advance(int count) => this.offset += count;
    ...
    public int ReadInt8() => (int) (sbyte) this.ReadUInt8();
    ...
    public int ReadInt16() => (int) (short) this.ReadUInt16();
    ...
    public int ReadUInt31()
    {
      ...
      return num >= 0 ? num : throw new Exception("XmlExceptionHelper.ThrowInvalidBinaryFormat(this.reader);");
    }

    public long ReadInt64()
    {
      long num = (long) (uint) this.ReadInt32();
      return ((long) (uint) this.ReadInt32() << 32) + num;
    }
    ...
    public TimeSpan ReadTimeSpan() => TimeSpan.FromTicks(this.ReadInt64());
    ...
    public int GetEscapedChars(int iOffset, int iLength, char[] iChars)
    {
      ...
      while (true)
      {
        while (iOffset < num1 && this.IsAttrChar((int) this.buffer[this.offset]))
          ++iOffset;
        escapedChars = iCharOffset + this.GetChars(iOffset1, iOffset - iOffset1, iChars, iCharOffset);
        if (iOffset != num1)
        {
          ...
          switch (buffer[iOffset])
          {
            ...
            case 38:
              ...
              if (charEntity <= (int) ushort.MaxValue)
              {
                ...
                int num3 = escapedChars;
                iCharOffset = num3 + 1;
                int index2 = num3;
                int num4 = (int) (ushort) charEntity;
...
label_16:
    ...
    public int GetCharEntity(int iOffset, int length)
    {
      ...
      {
        case 35:
          return buffer[iOffset + 2] == (byte) 120 ? this.GetHexCharEntity(iOffset, length) : this.GetDecimalCharEntity(iOffset, length);
        case 97:
          return buffer[iOffset + 2] == (byte) 109 ? this.GetAmpersandCharEntity(iOffset, length) : this.GetApostropheCharEntity(iOffset, length);
        case 103:
    ...
    {
      byte[] buffer = this.buffer;
      int decimalCharEntity = 0;
      for (int index = 2; index < length - 1; ++index)
      {
        byte num = buffer[this.offset + index];
        if (num < (byte) 48 || num > (byte) 57)
          throw new Exception("XmlExceptionHelper.ThrowInvalidCharRef(this.reader);");
        decimalCharEntity = decimalCharEntity * 10 + ((int) num - 48);
        if (decimalCharEntity > 1114111)
          throw new Exception("XmlExceptionHelper.ThrowInvalidCharRef(this.reader);");
      }
      return decimalCharEntity;
    }

    public double GetDouble(int iOffset) => BitConverter.ToDouble(this.Buffer, iOffset);

    public float GetSingle(int iOffset) => BitConverter.ToSingle(this.buffer, iOffset);
    ...
    public Decimal GetDecimal(int iOffset)
    {
      ...
      {
        (int) buffer[0 + iOffset] | (int) buffer[1 + iOffset] << 8 | (int) buffer[2 + iOffset] << 16 | (int) buffer[3 + iOffset] << 24,
    ...
    private int GetHexCharEntity(int iOffset, int length)
    {
      ...
      {
        byte num1 = buffer[this.offset + index];
        int num2;
        ...
        {
          if (num1 < (byte) 65 || num1 > (byte) 70)
            throw new Exception(" XmlExceptionHelper.ThrowInvalidCharRef(this.reader);");
          num2 = 10 + ((int) num1 - 65);
        }
        hexCharEntity = hexCharEntity * 16 + num2;
        if (hexCharEntity > 1114111)
          throw new Exception(" XmlExceptionHelper.ThrowInvalidCharRef(this.reader);");
      }
      return hexCharEntity;
    }
    ...
    public byte GetByte(int iOffset) => this.buffer[iOffset];

    public int GetInt8(int iOffset) => (int) (sbyte) this.GetByte(iOffset);
    ...
    public int GetInt32(int iOffset) => BitConverter.ToInt32(this.buffer, iOffset);

    public long GetInt64(int iOffset) => BitConverter.ToInt64(this.buffer, iOffset);

    public ulong GetUInt64(int iOffset) => (ulong) this.GetInt64(iOffset);

    public UniqueId GetUniqueId(int iOffset) => new UniqueId(this.buffer, iOffset);
    ...
    public void SkipNodeType() => this.SkipByte();
  }
}
