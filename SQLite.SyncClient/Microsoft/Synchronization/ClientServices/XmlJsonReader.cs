...
namespace Microsoft.Synchronization.ClientServices
{
  public class XmlJsonReader : XmlReader
  {
    ...
    {
      get => this.depth == 0 ? this.rootElementNode : this.elementNodes[this.depth];
    }
    ...
    public override bool HasValue => this.Node.HasValue;

    public override bool IsDefault => false;

    public override string this[int index] => this.GetAttribute(index);
    ...
    {
      get => this.GetAttribute(iLocalName, namespaceUri);
    }

    public override int AttributeCount => this.Node.CanGetAttribute ? this.attributeCount : 0;

    public override sealed int Depth => this.depth + this.Node.DepthDelta;

    public override bool EOF => this.Node.ReadState == ReadState.EndOfFile;

    public override sealed bool IsEmptyElement => this.Node.IsEmptyElement;
    ...
    public override sealed XmlNodeType NodeType => this.Node.NodeType;

    public override string Prefix => string.Empty;

    public override ReadState ReadState => this.Node.ReadState;

    private bool IsAttributeValue
    {
      get => this.Node.NodeType == XmlNodeType.Attribute || this.Node is XmlAttributeTextNode;
    }

    private bool IsReadingCollection
    {
      get => this.scopeDepth > 0 && this.scopes[this.scopeDepth] == JsonNodeType.Collection;
    }

    private bool IsReadingComplexText
    {
      get => !this.Node.IsAtomicValue && this.Node.NodeType == XmlNodeType.Text;
    }

    public override string Value => this.value ?? (this.value = this.Node.ValueAsString);
    ...
    {
      this.quotas = xmlQuotas != null ? xmlQuotas : throw new ArgumentNullException("quotas");
    ...
    {
      if (this.depth == 0)
        this.rootElement = !this.rootElement ? true : throw new XmlException("ThrowMultipleRootElements");
      ++this.depth;
    ...
    protected XmlAttributeNode AddAttribute() => this.AddAttribute(true);

    protected XmlAttributeNode AddXmlAttribute() => this.AddAttribute(true);
    ...
    private XmlAttributeNode GetAttributeNode(int index)
    {
      ...
      if (index < 0)
        throw new ArgumentOutOfRangeException(nameof (index));
      return index < this.attributeCount ? this.attributeNodes[index] : throw new ArgumentOutOfRangeException(nameof (index));
    }
    ...
    private XmlAttributeNode GetAttributeNode(string name)
    {
      ...
      int key = this.attributeStart;
      for (int index = 0; index < this.attributeCount; ++index)
      {
        if (++key >= this.attributeCount)
          key = 0;
        XmlAttributeNode attributeNode = attributeNodes[key];
        if (attributeNode.IsPrefixAndLocalName(prefix, slocalName))
        {
          this.attributeStart = key;
    ...
    private XmlAttributeNode GetAttributeNode(string iLocalName, string namespaceUri)
    {
      ...
      for (int index = 0; index < this.attributeCount; ++index)
      {
        ...
        if (key >= this.attributeCount)
          key = 0;
        XmlAttributeNode attributeNode = attributeNodes[key];
        if (attributeNode.IsLocalNameAndNamespaceUri(iLocalName, namespaceUri))
        {
          this.attributeStart = key;
          return attributeNode;
        }
    ...
    public override string GetAttribute(string name)
    {
      ...
      if (attributeNode == null)
        return (string) null;
      return name != "type" ? this.UnescapeJsonString(attributeNode.ValueAsString) : attributeNode.ValueAsString;
    }
    ...
    public override string GetAttribute(string iLocalName, string namespaceUri)
    {
      ...
      if (attributeNode == null)
        return (string) null;
      return iLocalName != "type" ? this.UnescapeJsonString(attributeNode.ValueAsString) : attributeNode.ValueAsString;
    }

    public override string LookupNamespace(string iPrefix) => "http://www.w3.org/2000/xmlns/";
    ...
    public override bool IsStartElement(string name)
    {
      ...
      return (this.Node.NodeType == XmlNodeType.Element || this.IsStartElement()) && this.Node.LocalName == str;
    }

    public override bool IsStartElement(string slocalName, string namespaceUri)
    {
      return slocalName != null && namespaceUri != null && (this.Node.NodeType == XmlNodeType.Element || this.IsStartElement()) && this.Node.LocalName == slocalName && this.Node.IsNamespaceUri(namespaceUri);
    }
    ...
    {
      this.BufferReader.SkipByte();
      char ch = (char) this.BufferReader.GetByte();
      if (ch == 'u')
      {
        this.BufferReader.SkipByte();
        int outOffset;
        ...
        this.BufferReader.Advance(4);
        int num = (int) XmlJsonReader.ParseChar(str1, NumberStyles.HexNumber);
        if (char.IsHighSurrogate((char) num) && this.BufferReader.GetByte() == (byte) 92)
        {
          this.BufferReader.SkipByte();
        ...
        if (buffer[outOffset + 4] == (byte) 34)
        {
          this.BufferReader.SkipByte();
          if (moveToText)
            this.MoveToAtomicText().Value.SetCharValue(num);
          this.complexTextMode = JsonComplexTextMode.None;
        }
        else
        {
          if (moveToText)
            this.MoveToComplexText().Value.SetCharValue(num);
        ...
        switch (ch)
        {
          ...
          case 'f':
            ...
            if (this.BufferReader.GetByte() == (byte) 34)
            {
              this.BufferReader.SkipByte();
              if (moveToText)
                this.MoveToAtomicText().Value.SetCharValue((int) ch);
              this.complexTextMode = JsonComplexTextMode.None;
            }
            else
            {
              if (moveToText)
                this.MoveToComplexText().Value.SetCharValue((int) ch);
              this.complexTextMode = JsonComplexTextMode.QuotedText;
            }
            break;
          case 'b':
    ...
    public override object ReadContentAs(Type type, IXmlNamespaceResolver namespaceResolver)
    {
      ...
      return type == typeof (string) ? (object) this.ReadContentAsString() : base.ReadContentAs(type, namespaceResolver);
    }
