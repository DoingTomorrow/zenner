...
namespace System.Data.SqlServerCe
{
  public sealed class SqlCeCommand : DbCommand, ICloneable
  {
    ...
    internal SqlCeTransaction InternalTransaction
    {
      get
      {
        return this.connection != null && this.connection.HasDelegatedTransaction ? this.connection.DelegatedTransaction.SqlCeTransaction : this.transaction;
      }
    }
    ...
    private static int GetParameterSize(MetaData info, SqlCeParameter p)
    {
      ...
      if (info.typeMap.fixlen < 0)
      {
        ...
        if (flag1 && !isBlob && flag2)
          parameterSize = p.Size != 0 ? Math.Min((p.Size + 1) * 2, (int) info.size + 2) : (int) info.size + 2;
        else if (p.Size == 0)
        {
          int parameterLength = p.GetParameterLength();
          parameterSize = !flag1 ? Math.Min(parameterLength, (int) info.size) : Math.Min((parameterLength + 1) * 2, (int) info.size + 2);
        }
        else
          parameterSize = !flag1 ? Math.Min(p.Size, (int) info.size) : Math.Min((p.Size + 1) * 2, (int) info.size + 2);
      }
      else
        parameterSize = info.typeMap.fixlen;
      return parameterSize;
    }

    public override void Cancel()
    {
      throw new NotSupportedException(string.Format((IFormatProvider) CultureInfo.InvariantCulture, nameof (Cancel)));
    }
    ...
    {
      return this.ExecuteResultSet(options, (SqlCeResultSet) null);
    }
    ...
    private void FillParameterDataBindings(bool verifyValue)
    {
      ...
      for (int index = 0; index < count; ++index)
      {
        ...
        if (parameterValue == null)
          throw new ArgumentNullException(sqlCeParameter.ParameterName, string.Format((IFormatProvider) CultureInfo.CurrentCulture, Res.GetString("ADP_ParameterNotSpecified"), (object) this.commandText));
        try
        {
    ...
    private MetaData[] GetQueryParameters(IntPtr pQpCommand)
    {
      uint columnCount = 0;
      IntPtr zero = IntPtr.Zero;
      MetaData[] queryParameters = (MetaData[]) null;
      try
      {
        ...
        queryParameters = new MetaData[(IntPtr) columnCount];
        QPPARAMINFO structure = new QPPARAMINFO();
        ...
        while ((long) num2 < (long) columnCount)
        {
          try
          {
            Marshal.PtrToStructure(ADP.IntPtrOffset(zero, offset), (object) structure);
            string lower = Marshal.PtrToStringUni(structure.pwszParam).ToLower(CultureInfo.CurrentCulture);
            SqlCeType typeMap = SqlCeType.FromSeType(structure.type);
            uint maxLength = typeMap.SqlDbType == SqlDbType.NVarChar || typeMap.SqlDbType == SqlDbType.NChar ? structure.ulSize / 2U : structure.ulSize;
            MetaData metaData = new MetaData(lower, typeMap, structure.bPrecision, structure.bScale, (long) maxLength, (string) null, (string) null);
            metaData.ordinal = structure.iOrdinal;
            metaData.size = structure.ulSize;
            metaData.typeMap = typeMap;
            queryParameters[(IntPtr) metaData.ordinal] = metaData;
          }
          finally
          {
            NativeMethods.SafeDelete(ref structure.pwszParam);
          }
          ++num2;
          offset += num1;
        }
