...
// Type: System.Reactive.Joins.ActivePlan`10
...
namespace System.Reactive.Joins
{
  internal class ActivePlan<T1, T2, T3, T4, T5, T6, T7, T8, T9, T10> : ActivePlan
  {
    ...
    {
      if (this.first.Queue.Count <= 0 || this.second.Queue.Count <= 0 || this.third.Queue.Count <= 0 || this.fourth.Queue.Count <= 0 || this.fifth.Queue.Count <= 0 || this.sixth.Queue.Count <= 0 || this.seventh.Queue.Count <= 0 || this.eighth.Queue.Count <= 0 || this.ninth.Queue.Count <= 0 || this.tenth.Queue.Count <= 0)
        return;
      ...
      {
        this.Dequeue();
        this.onNext(notification1.Value, notification2.Value, notification3.Value, notification4.Value, notification5.Value, notification6.Value, notification7.Value, notification8.Value, notification9.Value, notification10.Value);
      }
    }
  }
}
