...
// Type: System.Reactive.Joins.ActivePlan`5
...
namespace System.Reactive.Joins
{
  internal class ActivePlan<T1, T2, T3, T4, T5> : ActivePlan
  {
    ...
    {
      if (this.first.Queue.Count <= 0 || this.second.Queue.Count <= 0 || this.third.Queue.Count <= 0 || this.fourth.Queue.Count <= 0 || this.fifth.Queue.Count <= 0)
        return;
      ...
      {
        this.Dequeue();
        this.onNext(notification1.Value, notification2.Value, notification3.Value, notification4.Value, notification5.Value);
      }
    }
  }
}
